...
namespace Code.MVC.Configs
{
    public class PackageSettingsWindow : EditorWindow
    {
        ...
        {
            GetWindow<PackageSettingsWindow>("MVC Settings").Show();
        }

        private void OnEnable()
        {
            _settings = MVCSettingsLocator.FindOrCreate();
        }
        ...
        private void OnGUI()
        {
            ...
            bool newUseZenject = EditorGUILayout.Toggle("Use Zenject", _settings.useZenject);
            string newAdditional = EditorGUILayout.TextField("Additional Option", _settings.additionalOption);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(_settings, "Change MVC Settings");
                _settings.useZenject = newUseZenject;
                _settings.additionalOption = newAdditional;

                EditorUtility.SetDirty(_settings);

                // Safe to call directly here (not during import)
                MVCDefineSynchronizer.SyncFromSettings(_settings);
            }
            ...
            GUILayout.Space(6);
            if (GUILayout.Button("Save Settings Asset"))
            {
